...
class JobService:
                ...
                try:
                    ...
                    if not status:
                        ...
                        if video_files:
                            ...
                            status = "processing"
                        elif (job_dir / f"{job_id}_extraction.json").exists() or list(
                            job_dir.glob("*_extraction.json")
                        ):
                            status = "processing"
                        else:
                            status = "pending"
                    ...
                    if progress_value is not None:
                        logger.info(
                            f"Loaded job {job_id} with progress: {progress_value}%"
                        )
    ...
    def update_job(
        ...
        with self.lock:
            ...
            if progress is not None:
                logger.info(
                    f"Job {job_id}: Progress updated to {progress}% - {message}"
                )
            ...
            if status in ["completed", "failed"]:
                ...
                if s3_manager.s3:
                    ...
                    logger.warning(
                        f"S3 not configured. Keeping local files for job {job_id}"
                    )

    def get_job(self, job_id: str) -> Optional[Dict[str, Any]]:
        """Get job by ID - always reload from disk to get latest status from Celery worker."""
        with self.lock:
            # CRITICAL: Reload job metadata from disk to get latest status
            # This ensures we see updates from Celery worker process
            job_dir = settings.JOBS_OUTPUT_PATH / job_id
            metadata_path = job_dir / "job_metadata.json"
            ...
            if metadata_path.exists():
                try:
                    ...
                    # Update in-memory cache with latest data from disk
                    ...
                    logger.warning(f"Failed to reload job {job_id} from disk: {e}")

            # Fallback to in-memory data if file doesn't exist
    ...
    def list_jobs(self, limit: int = 10, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        with self.lock:
            sorted_jobs = sorted(
                self.jobs.values(), key=lambda x: x.get("created_at", ""), reverse=True
            )
